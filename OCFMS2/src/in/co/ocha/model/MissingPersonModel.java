...
public class MissingPersonModel extends BaseModel {
	...
	public long add() throws ApplicationException {
		...
		try {
			...
			PreparedStatement pstmt = conn
					.prepareStatement("INSERT INTO "+getTableName()+" (ID,POLICE_ST_ID,NAME,AGE,GENDER,HEIGHT,"
							+ "DATE_OF_MISSING,DATE_OF_REPORTING,COMPLEXION,"
							+ "HAIR,MARK_OF_IDENTIFICATION,AREA_OF_MISSING,REPORT_ID)"
							+ " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			pstmt.setString(11, markOfIdentification);
	...
	public MissingPersonModel findByPK(long pk) throws ApplicationException {
		...
		try {
			...
			while (rs.next()) {
				...
				model.setMarkOfIdentification(rs.getString(11));
	...
	public void update() throws ApplicationException {
		...
		try {
			...
			PreparedStatement pstmt = conn
					.prepareStatement("UPDATE "+getTableName()+"  SET POLICE_ST_ID =?,NAME = ?,AGE= ?,"
			...
			pstmt.setString(10, markOfIdentification);
			pstmt.setString(11, areaOfMissing);
			pstmt.setLong(12, reportId);
			//pstmt.setString(13, photo); --- need to add photo column into query as well
			pstmt.setLong(13, id);
	...
	public List search(int pageNo, int pageSize) throws ApplicationException {
		...
		if (policeStId != 0 && policeStId > 0) {
			sql.append(" AND POLICE_ST_ID like '" + policeStId);
		}
		...
		if (areaOfMissing != null && areaOfMissing.length() > 0) {
			sql.append(" AND AREA_OF_MISSING like '" + areaOfMissing);
		}

		if (reportId != 0 && reportId > 0) {
			sql.append(" AND REPORT_ID like '" + reportId);
		}
