...
import in.co.ocha.model.FeedbackModel;
...
public class MailCtl extends BaseCtl {
	...
	protected void doPost(HttpServletRequest request,
		...
		log.debug("MailCtl Method doPost Started");
	...
	protected void doGet(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		System.out.println("Mail do get start");
		...
		if (id > 0) {
			try {
				...
				ServletUtility.setModel(model, request);
				System.out.println("email is here "+model.getSender()+" "+model.getId()+" "+model.getDetail()
				+" "+model.getAttachment());
			} catch (ApplicationException e) {
				ServletUtility.handleException(e, request, response);
				return;
